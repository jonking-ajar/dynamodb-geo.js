...
export class DynamoDBManager {
  ...
  public async queryGeohash(
    queryInput: DynamoDB.DocumentClient.QueryInput | undefined,
    hashKey: Long,
    range: GeohashRange
    ...
    const nextQuery = async (lastEvaluatedKey: DynamoDB.Key = null) => {
      const keyConditions: {
        [key: string]: DynamoDB.DocumentClient.Condition;
      } = {};
      ...
      const minRange: DynamoDB.DocumentClient.AttributeValue = parseFloat(
        range.rangeMin.toString(10)
      );
      const maxRange: DynamoDB.DocumentClient.AttributeValue = parseFloat(
      ...
      const queryOutput = await this.config.documentClient
  ...
  public putPoint(
    ...
    putItemInput.Item[this.config.geohashAttributeName] = parseFloat(
      geohash.toString(10)
    );
  ...
  public batchWritePoints(
    ...
    putPointInputs.forEach((putPointInput) => {
      ...
      putRequest.Item[this.config.rangeKeyAttributeName] =
        putPointInput.RangeKeyValue;
      putRequest.Item[this.config.geohashAttributeName] = parseFloat(
        geohash.toString(10)
      );
  ...
  public updatePoint(
    ...
    if (updatePointInput.UpdateItemInput)
